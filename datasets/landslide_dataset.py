...
def read_img(img_path, key):
    file = h5py.File(img_path)
    return file[key][:]

class LandSlideDataset(Dataset):
    def __init__(self, img_dir, is_training, mask_dir='', normal_id='',  diversity_transform=False, background_index=-1, transforms=None):
    ...
    def __getitem__(self, i):
        ...
        img_file = read_img(img_path=img_file_path, key='img')

        mask_file_name = file_name.replace('image', 'mask')
        mask_file_path = os.path.join(self.mask_dir, mask_file_name)

        mask_file = read_img(img_path=mask_file_path, key='mask')
